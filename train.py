...
def train(train_loader, net, optimizer, epoch, scheduler):
    ...
    for idx, (images, patient_info, targets) in enumerate(train_loader):
        data_time.update(time.time() - end)
        ...
        # 通过网络进行前向传播
        outputs = net(images, patient_info)
...
def validate(val_loader, net):
    ...
    for idx, (images, patient_info, targets) in enumerate(val_loader):
        images, targets = images.to(device), targets.to(device)
        patient_info = patient_info.to(device)
